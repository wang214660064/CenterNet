...
# v9单变量A/B：从v5最佳权重出发，只训练Gate并开启双目光照增强。
python src/main.py stereo_ddd \
  --dataset kitti \
  --kitti_split project2000 \
  --arch stereo_dla_34 \
  --exp_id stereo_project2000_gate_photo_aug_v9 \
  --load_model exp/stereo_ddd/stereo_project2000_projected_center_v5_continue_e11/model_best.pth \
  --train_gate_only \
  --stereo_photo_aug \
  --stereo_photo_aug_prob 0.5 \
  --stereo_camera_mismatch_prob 0.15 \
  --stereo_photo_aug_strength 0.15 \
  --batch_size 16 \
  --num_workers 8 \
  --val_num_workers 8 \
  --lr 5e-5 \
